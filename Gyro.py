...
b.write_byte_data(LGD, LGD_CTRL_1, 0x3F) #turn on gyro and set to normal mode
b.write_byte_data(LGD, LGD_CTRL_4, 0x10) #scale of 500 dps
...
while True:
 ...
 #   accz = twos_comp_combine(b.read_byte_data(LSM, LSM_ACC_Z_MSB), b.read_byte_data(LSM, LSM_ACC_Z_LSB))
    #data = b.read_i2c_block_data(0x6b , 0x28 , 6)
    data = []
    data += [b.read_byte_data(LGD , 0x28)]
    data += [b.read_byte_data(LGD , 0x29)]
    data += [b.read_byte_data(LGD , 0x2a)]
    data += [b.read_byte_data(LGD , 0x2b)]
    data += [b.read_byte_data(LGD , 0x2c)]
    data += [b.read_byte_data(LGD , 0x2d)]
    gyrox = twos_comp_combine(data[1] , data[0])
    gyroy = twos_comp_combine(data[3] , data[2])
    gyroz = twos_comp_combine(data[5] , data[4])

    data_out = (gyrox, gyroy, gyroz)
